layout_with_coulomb <- function(edgelist,
    weights = NULL,
    sizes = NULL,
    vcount = NULL,
    niter = NULL,
    ...
    if (is.null(init)) {
        ...
        init <- matrix(0, nrow = n, ncol = 2) # this is inefficient if there are many nodes
        tl <- n + 1
    ...
    if (any(duplicated(init))) {
        warning("Duplicated initial placement found. Initial slightly pertubated.")
    ...
    if (is.null(sizes)) {
        sizes <- rep(1, n) 
        coulomb.rad <- 1 # force radius of 1 to not affect graph
    } else {
        # normalize to minimum
        sizes <- sizes / min(sizes)
        if (is.null(coulomb.rad)) coulomb.rad <- max(sizes) # default radius for coulombic force
    }
    ...
    if (version == 1) {
        ...
        layout <- layout_with_coulomb_Cpp(
            ...
            pcoulombrad = as.double(coulomb.rad),
            Ef = Ef,
            Et = Et,
            W = abs(weights),
            sizes = sizes,
